(ns oecd.main
  "Kotodama WASM entrypoint for the OECD clean-room actor (L4) — Clojure port.
  ...
  facts (`oecd.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"oecd.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "oecd-compat" :tier tier :entities entities} 200])
